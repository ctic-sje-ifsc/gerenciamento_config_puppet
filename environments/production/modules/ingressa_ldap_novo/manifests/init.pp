...
if $::kernel == windows {
	...
	if $lsbdistcodename == trusty {
		#PACOTES NECESSARIOS
		$list = [ "libnss-ldap", "libpam-ldap", "ldap-utils", "nscd", "smbclient", "libpam-mount", "libpam-ccreds", "nss-updatedb", "smbnetfs", "cifs-utils", "ldap-auth-config", "ldap-auth-client",  "auth-client-config", "keyutils", "libcryptsetup4", "libhx28" ]
  	package { $list:
		ensure => latest,
 ...
 file { 'common-password':
    ...
    source => 'puppet:///modules/ingressa_ldap/nssupdate.sh',
    owner => root,
    group => root,
    mode => 0755,
  }
	...
	if $lsbdistcodename == jessie {
		#PACOTES NECESSARIOS
		$list = [ "smbnetfs", "libpam-ldap", "libpam-ccreds", "libnss-ldap", "nss-updatedb", "libpam-mount", "cifs-utils", "cups-client" ]
		package { $list:
		ensure => latest,
		install_options => ['--allow-unauthenticated', '-y', '--force-yes'],
		}
	...
	file { 'client.conf':
		...
		source => 'puppet:///modules/ingressa_ldap/debian.client.conf',
		owner => root,
		group => root,
		mode => 0644,
	}
