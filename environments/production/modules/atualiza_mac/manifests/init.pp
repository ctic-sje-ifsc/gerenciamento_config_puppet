...
#}
	...
	exec { 'atualiza_mac':
 		command => "/bin/mv /home/backup/aluno.tgz /home/backup/aluno-$(date +'%d-%m-%Y').tgz ; /bin/tar -zcvf /home/backup/aluno.tgz /home/aluno ; /usr/bin/touch /var/gatilho_tar_1; /bin/rm -rf /var/gatilho_tar",
 		timeout => 0,
 		require => exec[atualiza_tar],
 		creates => '/var/gatilho_tar_1',
  }
  exec { 'atualiza_tar':
